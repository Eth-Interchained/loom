...
13. The signal handler must **NOT** set `SA_ONSTACK`. Rust's runtime installs a few-KB per-thread `sigaltstack`; with `SA_ONSTACK` the kernel runs our handler there and the resolve path (which calls into `Loom`) overflows it. A stack overflow inside a SIGSEGV handler is unrecoverable — instant process death. **Measured: the debug build died on the first arena fault while release survived**, because release frames fit. `cargo test` in **debug** is what catches this, so CI must keep running the debug tier.
14. The signal handler preallocates its own bookkeeping (`state`, `ring`, `staging`) and takes no lock the faulting thread could hold. Note the honest limit: the resolve path calls `Loom::read`/`write`, **which can allocate** (the prefetch batcher's `Vec`). Pass `prefetch_depth: Some(0)` to a `Loom` under a fault arena. The real fix for both this and the stack risk is a notify-and-wait handler with a worker thread — not built.
15. The signal handler **chains** to the saved previous disposition for any fault outside `[BASE, END)` and for any fault it cannot resolve. Swallowing another subsystem's `SIGSEGV` silently breaks crash reporting process-wide. `Drop` disarms (`BASE = 0`) *before* restoring dispositions and unmapping.
16. When stacking a fault arena on a `Loom`, the `Loom` frame budget must be SMALL — the arena's mapped pages are the cache; Loom is the I/O path. Equal budgets stack and double the footprint (measured 257.8 MiB for a stated 128 MiB).
17. An option a caller passes must reach the object they get. `Loom::create` delegates to `create_with`; substituting defaults on the create path made a "prefetch disabled" measurement report prefetch running.
